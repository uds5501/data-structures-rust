...
impl<'a> ProtoMessage<'a> for PhoneNumber<'a> {
    fn add_field(&mut self, field: Field<'a>) -> Result<(), Error> {
        match field.field_num {
            1 => {
                self.number = field.value.as_string().unwrap();
            }
            2 => {
                self.type_ = field.value.as_string().unwrap();
            }
            _ => {}
        }
        Ok(())
...
impl<'a> ProtoMessage<'a> for Person<'a> {
    fn add_field(&mut self, field: Field<'a>) -> Result<(), Error> {
        match field.field_num {
            1 => {
                self.name = field.value.as_string().unwrap();
            }
            2 => {
                self.id = field.value.as_u64().unwrap();
            }
            3 => {
                let mut p: PhoneNumber = Default::default();
                let _ = p.add_field(field);
                self.phone.push(p)
            }
            _ => {}
        }
